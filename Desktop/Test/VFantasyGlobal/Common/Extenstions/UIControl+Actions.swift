...
extension UIControl {
     ...
     */
    @discardableResult
    func add<T: UIControl>(event: UIControl.Event, action: @escaping (T, UIEvent?) -> Void) -> Action {
     ...
     */
    @discardableResult
    func add<T: UIControl>(event: UIControl.Event, action: @escaping (T) -> Void) -> Action {
     ...
     */
    @discardableResult
    func add(event: UIControl.Event, action: @escaping () -> Void) -> Action {
     ...
     */
    @discardableResult
    func add<T: UIControl>(events: [UIControl.Event], action: @escaping (T, UIEvent?) -> Void) -> [Action] {
     ...
     */
    @discardableResult
    func addAction<T: UIControl>(events: [UIControl.Event], action: @escaping (T) -> Void) -> [Action] {
        return events.map { add(event: $0, action: action) }
    }
     ...
     */
    @discardableResult
    func addAction(events: [UIControl.Event], action: @escaping () -> Void) -> [Action] {
        return events.map { add(event: $0, action: action) }
    }
    ...
    func remove(action: Action, forControlEvents events: UIControl.Event) {
    ...
    /**
     ...
     - parameter events: The control events that you want to remove for the specified target object
     */
    func remove(_ action: Action, for events: UIControl.Event) {
