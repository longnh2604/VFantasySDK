...
protocol CreateTeamInfoView : BaseViewProtocol {
...
class CreateTeamInfoPresenter: NSObject {
    weak private var view: CreateTeamInfoView?
    ...
    private let playerService = CreateTeamInfoService()
...
//    sds
    ...
    func attachView(view: CreateTeamInfoView) {
        self.view = view
    ...
    func refreshPlayerList(_ toggleSort: Bool = false) {
        ...
        let transferRequestModel = TransferRequestModel(property: "transfer_value", direction: currentSortType.rawValue)
    ...
    private func handleLineupResponseSuccess(_ response: AnyObject) {
    ...
    func handleCompleteLineupResponseSuccess(_ response: AnyObject) {
        if let result = response as? LeagueTeamsData {
            ...
            view?.finishLoading()
            view?.completeLineup()
            view?.reloadView(currentPage)
        }
    }
}

extension CreateTeamInfoPresenter : CustomPageControlDelegate {
